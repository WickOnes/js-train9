...
function findElementGreaterThan(arr, num) {
  ...
  return !Array.isArray(arr) ? undefined : arr.find((el) => el > num);
}
...
function getPositiveArrayValues(arr) {
  ...
  return !Array.isArray(arr)
    ? "Вхідний параметр має бути масивом"
    : Array.from(arr.values()).filter((value) => value > 0);
}
...
function removeAndDouble(arr, startIdx) {
  ...
  arr.splice(startIdx);
  const doubledValues = arr.map((value) => value * 2);
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((previousValue, currentValue) => {
...
function capitalizeWords(arr) {
  ...
  const capitalizedWords = arr.map(
